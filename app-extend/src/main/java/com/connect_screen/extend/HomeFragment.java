...
public class HomeFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        Button forcePortraitBtn = view.findViewById(R.id.forcePortraitBtn);
        ...
        // 强制外接屏竖屏：找到所有物理外接屏，冻结旋转为 90° / 270°
        forcePortraitBtn.setOnClickListener(v -> {
            if (!ShizukuUtils.hasPermission()) {
                Toast.makeText(requireContext(), "需要先授权 Shizuku", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            java.util.List<Display> external = new java.util.ArrayList<>();
            ...
            if (external.isEmpty()) {
                Toast.makeText(requireContext(), "未检测到外接屏幕，请先连接显示器", Toast.LENGTH_SHORT).show();
                return;
            }
            // 显示旋转选项
            int n = external.size();
            String[] items = { "竖屏 90°", "竖屏 270°", "横屏 0°", "恢复（取消强制）" };
            new AlertDialog.Builder(requireContext())
                .setTitle("选择方向（共 " + n + " 个外接屏）")
                .setItems(items, (dialog, which) -> {
                    int rotation;
                    if (which == 0) rotation = Surface.ROTATION_90;
                    else if (which == 1) rotation = Surface.ROTATION_270;
                    else if (which == 2) rotation = Surface.ROTATION_0;
                    else rotation = -1; // 恢复
                    for (Display d : external) {
                        State.startNewJob(new ChangeRotation(d.getDisplayId(), rotation));
                    }
                    String msg = rotation == -1 ? "已恢复自动旋转"
                               : rotation == Surface.ROTATION_0 ? "已强制横屏"
                               : "已强制竖屏";
